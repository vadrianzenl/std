...
public class PasswordEncoder {
    ...
    //Función para comparar password con encriptación de Java
    //public boolean matches(CharSequence rawPassword, String encodedPassword) {
    //byte[] digested = decode(encodedPassword);
    //byte[] salt = subArray(digested, 0, saltGenerator.getKeyLength());
    //return matches(digested, digest(rawPassword, salt));
    //}
    //Función para comparar password con encriptación de SIGA
    public boolean matches(String rawPassword, String encodedPassword) {
        boolean result = false;
        if (rawPassword.equals(Hex.decodeSiga(encodedPassword).trim())) {
            result = true;
        }
        return result;
    }
    ...
    public byte[] decode(CharSequence encodedPassword) {
